...
| `cargo test -p nils-google-cli --test native_no_gog` | PASS |
| `cargo test -p nils-google-cli --lib` | PASS |
...
1. Run `cargo run -p nils-google-cli -- auth add <email> --manual` and complete the loopback/manual flow.
2. Verify `cargo run -p nils-google-cli -- auth status --json` shows the expected default account.
...
1. Run `cargo run -p nils-google-cli -- gmail send --to <target> --subject "<subject>" --body "<body>"`.
...
1. Run `cargo run -p nils-google-cli -- drive upload <local-path> --name <smoke-name>`.
