...
| Local `auth save`/`auth remove`/`auth use` actions fail | Secret directory is unset/unwritable          | With `CODEX_AUTH_REMOTE_SSH` unset, set `CODEX_SECRET_DIR` to a writable directory (for example `~/.config/codex_secrets`), then retry. |
| Remote `auth login`/`auth save`/`auth remove` is disabled | Intentional access-only replica boundary | Manage login and saved profiles on the configured authority; use remote `cxau` only to sync an access-only profile to the replica. |
| Remote `cxau` fails without switching | Replica helper is missing/busy, the SSH authority is unreachable, or the profile is absent | Install the sympoies-infra replica helper, confirm `CODEX_AUTH_REMOTE_SSH`, and retry. Exit 75 means another shell/launchd/Alfred writer owns the shared lock. Remote mode never falls back to local `auth use`. |
