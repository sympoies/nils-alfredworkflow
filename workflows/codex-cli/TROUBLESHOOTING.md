...
| Remote `cxau` fails without switching | `CODEX_AUTH_REMOTE_SSH` is unreachable or the profile is absent on the authority | Confirm the SSH alias and authority profile, then retry. Remote mode fails closed and never falls back to local `auth use`. |
