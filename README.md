...
| [Google Service](workflows/google-service/README.md) | `gs`, `gsa` | `gs` shows current account status; `gsa` runs auth login/remove/switch for multi-account operation. | OAuth credentials in `google-cli` auth store (runtime is bundled) |
