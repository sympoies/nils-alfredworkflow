...
| [Weather Forecast](workflows/weather/README.md) | `wt`, `ww`, `weather` | Show single-city or multi-city today rows then hourly (`wt`) / city picker then 7-day (`ww`) forecasts, then copy selected rows. | Optional: `WEATHER_CLI_BIN`, `WEATHER_LOCALE`, `WEATHER_DEFAULT_CITIES`, `WEATHER_CACHE_TTL_SECS`, `PREFERENCE_PROJECTION_FILE` |
